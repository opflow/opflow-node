...
var Logger = function(kwargs) {
  ...
  var logIdTree = kwargs.logIdTree;
  var logIdTreeEnabled = kwargs.logIdTreeEnabled || process.env.OPFLOW_LOGIDTREE === 'true';
  ...
  self.log = function(level) {
    if (logIdTreeEnabled && arguments.length === 2) {
  ...
  var extendLogMeta = function(log) {
    if (logIdTree && log && typeof(log) === 'object') {
...
var LogIdTree = function(kwargs) {
  ...
  if (kwargs.root !== null && typeof(kwargs.root) === 'object') {
    this.defineRoot(kwargs.root);
  }
}
...
var LogAdapter = new (function() {
  ...
  var logIdTree = new LogIdTree({
    root: { id: misc.instanceId, type: 'instanceId' }
  });
  ...
  this.getLogger = function(kwargs) {
    ...
    kwargs = kwargs || {};
    kwargs.store = store;
    kwargs.logIdTree = logIdTree;
    kwargs.logIdTreeEnabled = kwargs.logIdTreeEnabled || (store.realLogger !== null);
  ...
  this.bootstrap = function() {
    var LX = this.getLogger({ scope: 'opflow:bootstrap' });
