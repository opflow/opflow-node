    ...
    var _doWith = function(ch) {
      return assertQueue(ch, config).then(function() {
        if (context.sendable !== false) {
          ...
          L.isEnabledFor('verbose') && L.log('verbose', '%s() channel is writable, msg has been sent', config.name);
        } else {
          ch.once('drain', function() {
            context.sendable =_sendTo(ch);
            L.isEnabledFor('verbose') && L.log('verbose', '%s() channel is drained, flushed', config.name);
          });
          L.isEnabledFor('verbose') && L.log('verbose', '%s() channel is overflowed, waiting', config.name);
        }
