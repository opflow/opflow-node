...
var Executor = function(params) {
  L.isEnabledFor('conlog') && L.log('conlog', ' + constructor begin ...');
    ...
    var _doWith = function(ch) {
      return assertQueue(ch, config).then(function() {
        if (context.sendable !== false) {
          ...
          L.isEnabledFor('conlog') && L.log('conlog', '%s() channel is writable, msg has been sent', config.name);
        } else {
          ch.once('drain', function() {
            context.sendable =_sendTo(ch);
            L.isEnabledFor('conlog') && L.log('conlog', '%s() channel is drained, flushed', config.name);
          });
          L.isEnabledFor('conlog') && L.log('conlog', '%s() channel is overflowed, waiting', config.name);
        }
  ...
  L.isEnabledFor('conlog') && L.log('conlog', ' - constructor end!');
}
