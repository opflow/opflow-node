...
var TimeoutHandler = function(kwargs) {
  ...
  var timerTask = function() {
    ...
    L1.isEnabledFor('conlog') && L1.log('conlog', currentTrail.add().toMessage({
      tags: ['TimeoutHandler checking loop'],
      text: 'TimeoutHandler checking loop is invoked'
    }));
      ...
      if (lodash.isObject(task)) {
        ...
        if (timeout > 0) {
          ...
          L1.isEnabledFor('conlog') && L1.log('conlog', currentTrail.add({
            ...
            text: 'TimeoutHandler check timeout for a task'
          }));
          if (timediff > timeout) {
            ...
            if (lodash.isFunction(raiseTimeout)) {
              ...
              L1.isEnabledFor('conlog') && L1.log('conlog', currentTrail.add({
                ...
                tags: ['TimeoutHandler raise timeout tasks'],
                text: 'TimeoutHandler task is timeout, event will be raised'
              }));
            } else {
              L1.isEnabledFor('conlog') && L1.log('conlog', currentTrail.add({
                taskId: taskId
              }).toMessage({
                text: 'TimeoutHandler task is timeout, task will be removed'
              }));
              delete config.tasks[taskId];
            }
          } else {
            L1.isEnabledFor('conlog') && L1.log('conlog', currentTrail.add({
              ...
              text: 'TimeoutHandler task in good status, keep running'
            }));
          }
        }
      }
    ...
    L1.isEnabledFor('conlog') && L1.log('conlog', currentTrail.add().toMessage({
      text: 'TimeoutHandler checking loop has been done'
    }));
  }
  ...
  var defaultTeardown = function(state) {
    ...
    lodash.forEach(lodash.keys(state.tasks), function(taskId) {
      L1.isEnabledFor('error') && L1.log('error', monitorTrail.add({
        ...
        text: 'TimeoutHandler will be stopped, SAVE UNFINISHED TASKS'
      }));
    });
  }
  ...
  this.start = function() {
    L1.isEnabledFor('info') && L1.log('info', monitorTrail.add().toMessage({
      text: 'TimeoutHandler daemon is starting'
    }));
    timer.start();
  }
  ...
  this.stop = function(opts) {
    ...
    timer.stop();
    L1.isEnabledFor('info') && L1.log('info', monitorTrail.add().toMessage({
      text: 'TimeoutHandler daemon will be stopped'
    }));
    opts.timeout = typeof(opts.timeout) === 'number' ? opts.timeout : 30000;
    if (opts.timeout <= 0) {
      L1.isEnabledFor('info') && L1.log('info', monitorTrail.add().toMessage({
        text: 'TimeoutHandler daemon has stopped immediately'
      }));
      return Promise.resolve();
    }
    return new Promise(function(onResolved, onRejected) {
      L1.isEnabledFor('info') && L1.log('info', monitorTrail.add().toMessage({
        text: 'TimeoutHandler daemon will be stopped in ' + opts.timeout
      }));
      ...
      var stopWaiting = function(ok) {
        ...
        L1.isEnabledFor('info') && L1.log('info', monitorTrail.add().toMessage({
          text: 'TimeoutHandler daemon has stopped'
        }));
        onResolved(ok === true);
      }
...
var RepeatedTimer = function(kwargs) {
  ...
  L2.isEnabledFor('info') && L2.log('info', monitorTrail.add().toMessage({
    text: 'RepeatedTimer.new()'
  }));
...
var PayloadReader = function(kwargs) {
  ...
  L4.isEnabledFor('info') && L4.log('info', payloadTrail.add().toMessage({
...
var PayloadWriter = function(next, kwargs) {
  ...
  L3.isEnabledFor('info') && L3.log('info', payloadTrail.add().toMessage({
