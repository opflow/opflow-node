...
var TimeoutHandler = function(kwargs) {
        ...
        if (timeout > 0) {
          ...
          if (timediff > timeout) {
            ...
            if (lodash.isFunction(raiseTimeout)) {
              ...
              L1.isEnabledFor('conlog') && L1.log('conlog', currentTrail.add({
                message: 'TimeoutHandler task is timeout, task will be removed',
                taskId: taskId
              }).toString({reset:true}));
              delete config.tasks[taskId];
            }
  ...
  this.add = function(taskId, task) {
    ...
    task.timestamp = task.timestamp || misc.getCurrentTime();
...
var L3 = LogAdapter.getLogger({ scope: 'opflow:task:CouplingStream' });

var CouplingStream = function(onNext, kwargs) {
  kwargs = kwargs || {};
  Writable.call(this, {objectMode: true});
  this._context = { count: 0, writable: true };
  this._notifier = new events.EventEmitter();
  this._onNext = onNext;
  this._logTrail = LogTracer.ROOT.branch({ key: 'streamId', value: kwargs.streamId });

  L3.isEnabledFor('info') && L3.log('info', this._logTrail.add({
    message: 'CouplingStream.new()'
  }).toString());

  L3.isEnabledFor('info') && L3.log('info', this._logTrail.add({
    message: 'CouplingStream.new() end!'
  }).toString());
}

util.inherits(CouplingStream, Writable);

CouplingStream.prototype.count = function() {
  return this._context.count;
}

CouplingStream.prototype._write = function(chunk, encoding, callback) {
  var self = this;
  if (self._context.writable) {
    self._next(chunk, callback);
  } else {
    self._notifier.once('drain', function() {
      self._next(chunk, callback);
    });
  }
}

CouplingStream.prototype._next = function(obj, callback) {
  var self = this;
  self._context.writable = self._onNext(obj, self._context.count, self._notifier);
  L3.isEnabledFor('info') && L3.log('info', self._logTrail.add({
    count: self._context.count++,
    drained: self._context.writable,
    message: 'object has been sent'
  }).toString());
  callback();
}
