...
var TimeoutHandler = function(kwargs) {
  ...
  var logMonitor = LogTracer.ROOT.branch({ key:'monitorId', value:config.monitorId });

  L1.isEnabledFor('info') && L1.log('info', logMonitor.add({
  ...
  var timerTask = function() {
    ...
    var logCurrent = logMonitor.branch({ key:'checktime', value:current.toISOString() })

    L1.isEnabledFor('conlog') && L1.log('conlog', logCurrent.add({
...
var RepeatedTimer = function(kwargs) {
  ...
  kwargs = kwargs || {};
  kwargs.monitorId = kwargs.monitorId || misc.getUUID();

  var logMonitor = LogTracer.ROOT.branch({ key:'monitorId', value:kwargs.monitorId });

  L2.isEnabledFor('info') && L2.log('info', logMonitor.add({
