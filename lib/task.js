...
var PayloadReader = function(next, kwargs) {
  kwargs = kwargs || {};
  ...
  L4.isEnabledFor('info') && L4.log('info', payloadTrail.add({
    message: 'PayloadReader.new()'
  }).toString());

  var doRead = function(_index) {
    if (_index >= 0 && _chunk[_index] !== undefined) {
      L4.isEnabledFor('trace') && L4.log('trace', payloadTrail.add({
        ...
        state: lodash.reduce(_chunk, function(accum, chunk, index) {
          ...
          accum.size += Buffer.byteLength(chunk);
          return accum;
        }, { total: 0, size: 0, index: [] }),
        message: 'doRead() - status'
      }).toString({reset:true}));
      self.push(_chunk[_index]);
      delete _chunk[_index];
      L4.isEnabledFor('info') && L4.log('info', payloadTrail.add({
        index: _index,
  ...
  self.doAdd = function(index, chunk) {
    if (index >= 0 && chunk && (_index <= index || _chunk[index] === undefined)) {
      L4.isEnabledFor('info') && L4.log('info', payloadTrail.add({
        index: index,
        message: 'doAdd() - inserted'
      }).toString());
      _chunk[index] = misc.bufferify(chunk);
      _max = (_max < index) ? index : _max;
      _notifier.emit('add', {});
    } else {
      L4.isEnabledFor('info') && L4.log('info', payloadTrail.add({
        index: index,
        message: 'doAdd() - skipped'
      }).toString());
    }
  }

  self.doEnd = function(err) {
    if (err) {
      self.emit('error', err);
