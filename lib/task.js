...
var CouplingStream = function(onNext, kwargs) {
  ...
  var streamTrail = LogTracer.ROOT.branch({ key: 'streamId', value: kwargs.streamId });

  var self = this;
  var _context = { count: 0, writable: true };
  var _notifier = new events.EventEmitter();
  var _onNext = onNext;
  var _next = function(chunk, callback) {
    if (kwargs.usePromise) {
      _onNext(chunk, _context.count++, _notifier).then(function(drained) {
        _context.writable = drained;
        L3.isEnabledFor('info') && L3.log('info', streamTrail.add({
          count: _context.count,
          drained: _context.writable,
          message: 'processing has done'
        }).toString());
        callback();
      }).catch(function(error) {
        L3.isEnabledFor('error') && L3.log('error', streamTrail.add({
          count: _context.count,
      ...
      try {
        _context.writable = _onNext(chunk, _context.count++, _notifier);
        L3.isEnabledFor('info') && L3.log('info', streamTrail.add({
          count: _context.count,
          drained: _context.writable,
          message: 'processing has done'
  ...
  this._write = function(chunk, encoding, callback) {
    if (_context.writable) {
      _next(chunk, callback);
    } else {
