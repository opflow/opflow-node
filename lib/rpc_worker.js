...
var RpcWorker = function(params) {
  LW.isEnabledFor('verbose') && LW.log('verbose', ' + constructor begin ...');
  ...
  params = lodash.defaults({mode: 'rpc_worker'}, params);
  ...
  LW.isEnabledFor('verbose') && LW.log('verbose', 'operatorName: %s', operatorName);
  ...
  LW.isEnabledFor('verbose') && LW.log('verbose', 'responseName: %s', responseName);
  ...
  this.process = function(checker, callback) {
    if (typeof(checker) === 'string') {
      ...
      LW.isEnabledFor('verbose') && LW.log('verbose', 'process() - filter is a string: %s', checker);
    }
    ...
    return consumerRef = consumerRef || engine.consume(function(msg, sandbox, done) {
      ...
      LW.isEnabledFor('verbose') && LW.log('verbose', 'Request[%s]/Routine[%s] - process() receive a message', 
          requestId, routineId);

      var result = Promise.reduce(middlewares, function(A, middleware, index) {
        LW.isEnabledFor('verbose') && LW.log('verbose', 'Request[%s] - check middleware[%s]',
          requestId, index);
        if (A.count > 0 && (A.nextAction == null || A.nextAction == 'done')) return A;
        if (middleware.checker(routineId)) {
          LW.isEnabledFor('verbose') && LW.log('verbose', 'Request[%s] - middleware[%s] matched',
            requestId, index);
          A.count += 1;
      ...
      result.then(function(info) {
        LW.isEnabledFor('verbose') && LW.log('verbose', 'Request[%s] - has been done: %s',
          requestId, JSON.stringify(info));
        done(null, info);
      }).catch(function(exception) {
        if (LW.isEnabledFor('verbose')) {
          LW.log('verbose', 'Request[%s] - exception: %s', requestId, JSON.stringify(exception));
  ...
  misc.notifyConstructor(asyncFuns, self);

  LW.isEnabledFor('verbose') && LW.log('verbose', ' - constructor end!');
}
