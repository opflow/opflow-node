...
var RpcWorker = function(params) {
  ...
  this.process = function(checker, callback) {
    ...
    if (checker instanceof Array) {
      ...
      LW.isEnabledFor('verbose') && LW.log('verbose', 'process() - filter is an array: %s', JSON.stringify(checker));
    }

    if (typeof(checker) === 'function') {
      middlewares.push({ checker: checker, listener: callback });
      LW.isEnabledFor('verbose') && LW.log('verbose', 'process() - filter is a function');
    }
    ...
    return consumerRef = consumerRef || engine.consume(function(msg, sandbox, done) {
      ...
      result.then(function(info) {
        LW.isEnabledFor('info') && LW.log('info', {
          requestId: requestId,
          extra: info,
          note: 'process() turn back with success'});
        done(null, info);
      }).catch(function(exception) {
        LW.isEnabledFor('info') && LW.log('info', {
          requestId: requestId,
          note: 'process() turn back with failure'});
