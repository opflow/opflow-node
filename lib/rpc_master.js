...
var RpcMaster = function(params) {
  ...
  var logTracer = LogTracer.ROOT.branch({key:'rpcMasterId',value:params.engineId});
  var store = { tasks: {} }

  LM.isEnabledFor('info') && LM.log('info', logTracer.add({
  ...
  this.request = function(routineId, data, opts) {
    ...
    var logRequest = logTracer.branch({ key:'requestId', value:opts.requestId });

    LM.isEnabledFor('info') && LM.log('info', logRequest.add({
  ...
  var initResponseConsumer = function(forked) {
    var logInitResponse = logTracer.copy();
    ...
    return engine.consume(function(msg, sandbox, done) {
      sandbox = sandbox || {};
      var logResponse = (sandbox.id) ? logTracer.branch({
      ...
      LM.isEnabledFor('conlog') && LM.log('conlog', logResponse.add({
      ...
      try {
        ...
        LM.isEnabledFor('info') && LM.log('info', logResponse.add({
  ...
  this.close = function() {
    ...
    LM.isEnabledFor('info') && LM.log('info', logClose.add({
  ...
  if (params.autoinit !== false) {
    LM.isEnabledFor('debug') && LM.log('debug', logTracer.add({
...
var RpcRequest = function RpcRequest(kwargs) {
  ...
  var logRequest = LogTracer.ROOT.branch({ key:'requestId', value:_requestId });
  ...
  this.raiseTimeout = function() {
    ...
    LR.isEnabledFor('debug') && LR.log('debug', logRequest.add({
  ...
  this.extractResult = function() {
    ...
    return new Promise(function(onResolved, onRejected) {
      ...
      task.on('started', function(info) {
        LR.isEnabledFor('conlog') && LR.log('conlog', logRequest.add({
...
var RpcResult = function(task, info) {
  ...
  var requestId = task.requestId || misc.getUUID();
  var logRequest = LogTracer.ROOT.branch({ key:'requestId', value:requestId });

  LT.isEnabledFor('info') && LT.log('info', logRequest.add({
