  ...
  self.subscribe = function(callback) {
    ...
    return engine.consume(function(msg, sandbox, done) {
      ...
      LX.isEnabledFor('info') && LX.log('info', requestTrail.toMessage({
        text: 'subscribe() receive a message'
      }));

      callback(msg.content, headers, function(err, result) {
        if (err) {
          LX.isEnabledFor('info') && LX.log('info', requestTrail.toMessage({
            text: 'consume() - processing failed, requeue the message'
          }));
          var strDone;
          ...
          if (props.headers['redeliveredCount'] <= redeliveredLimit) {
            LX.isEnabledFor('debug') && LX.log('debug', requestTrail.add({
              ...
              text: 'subscribe() - requeue to Subscriber'
            }));
            ...
            if (recyclebinName) {
              LX.isEnabledFor('debug') && LX.log('debug', requestTrail.add({
                ...
                text: 'subscribe() - enqueue to Recyclebin'
              }));
              ...
              LX.isEnabledFor('debug') && LX.log('debug', requestTrail.add({
                ...
                text: 'subscribe() - throw message away'
              }));
          ...
          strDone.finally(function() {
            LX.isEnabledFor('info') && LX.log('info', requestTrail.toMessage({
              text: 'turn back with failure'
            }));
            done(err);
          });
        } else {
          LX.isEnabledFor('info') && LX.log('info', requestTrail.toMessage({
            text: 'turn back with success'
          }));
          done(null);
        }
      });
