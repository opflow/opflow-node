  ...
  self.subscribe = function(callback) {
    ...
    return engine.consume(function(msg, sandbox, done) {
      ...
      L.isEnabledFor('info') && L.log('info', {
        requestId: requestId,
        note: 'Subscriber receive message'});
      callback(msg.content, headers, function(err, result) {
        if (err) {
          L.isEnabledFor('verbose') && L.log('verbose', {
            note: 'consume() - processing failed, requeue the message'});
          var strDone;
          ...
          if (props.headers['redeliveredCount'] <= redeliveredLimit) {
            L.isEnabledFor('info') && L.log('info', {
              ...
              note: 'subscribe() - requeue to Subscriber'});
            ...
            if (recyclebinName) {
              L.isEnabledFor('info') && L.log('info', {
                ...
                redeliveredCount: props.headers['redeliveredCount'],
                redeliveredLimit: redeliveredLimit,
                note: 'subscribe() - enqueue to Recyclebin'});
              ...
              L.isEnabledFor('info') && L.log('info', {
                ...
                redeliveredCount: props.headers['redeliveredCount'],
                redeliveredLimit: redeliveredLimit,
                note: 'subscribe() - throw message away'});
          ...
          strDone.finally(function() {
            L.isEnabledFor('info') && L.log('info', {
              requestId: requestId,
              note: 'turn back with failure'});
            done(err);
          });
        } else {
          L.isEnabledFor('info') && L.log('info', {
            requestId: requestId,
            note: 'turn back with success'});
          done(null);
        }
      });
