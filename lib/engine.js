...
var Engine = function(params) {
  ...
  this.exhaust = function(stream, props, override) {
    ...
    var ok = obtainProducerSandbox().then(function(sandbox) {
      ...
      var sendTo = function(body, index) {
        var segmentId = misc.getLogID();
        var segmentTrail = requestTrail.branch({ key: 'segmentId', value: segmentId });
        props.headers['segmentId'] = segmentId;
        props.headers['segmentIndex'] = index;
        if (sandbox.confirmable) {
          producerTasks[segmentId] = { body: body, props: props, override: override,
            timestamp: misc.getCurrentTime()
          }
        ...
        return sandbox.channel.publish(config.exchangeName, routingKey, 
          ...
          if (err) {
            ...
            delete producerTasks[segmentId];
            LX.isEnabledFor('debug') && LX.log('debug', segmentTrail.add({
      ...
      engineWatcher.once('close', function(args) {
        producerStream.emit('error', { message: 'Stream piping is interrupted' });
      })
