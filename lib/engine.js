...
var getProducerState = function() {
  return (this.producerState = this.producerState || {});
}

var lockProducer = function(override) {
  var self = this;
  var producerState = getProducerState.call(self);
  return retrieveExchange.call({ config: self.config, store: producerState }, override);
}
    ...
    ok = ok.then(function(qok) {
      ...
      return ch_consume(qok.queue, function(msg) {
        ...
        if (callback && typeof(callback) === 'function') {
          if (hasFeedback.call(self)) {
            var properties = lodash.assign({}, msg.properties, {headers: {error: false, status: 'started'}});
            enqueueFeedback.call(self, stringify({}), properties);
          }
          callback(msg.content, {
            ...
            if (hasFeedback.call(self)) {
              ...
              var properties;
              if (err) {
                properties = lodash.assign({}, msg.properties, {headers: {error: true, status: 'failed'}});
                debugx.enabled && debugx('consume() - processing failed: %s', stringify(err));
              } else {
                properties = lodash.assign({}, msg.properties, {headers: {error: false, status: 'completed'}});
                debugx.enabled && debugx('consume() - processing successful');
              }
              enqueueFeedback.call(self, stringify(result || err || {}), properties);
              debugx.enabled && debugx('consume() - enqueueFeedback send message back');
              ch.ack(msg);
            } else {
              ...
              if (hasRecycler.call(self)) {
                ...
                if (err) {
                  ...
                  if (headers[rcfg.redeliveredCountName] <= rcfg.redeliveredLimit) {
                    ...
                    enqueueConsumer.call(self, msg.content.toString(), {headers: headers});
                  } else {
                    debugx.enabled && debugx('consume() - enqueueRecycler message');
                    enqueueRecycler.call(self, msg.content.toString(), {headers: headers});
                  }
                }
                ch.ack(msg);
              } else {
            ...
            progress: function(completed, total, data) {
              if (hasFeedback.call(self)) {
                ...
                var properties = lodash.assign({}, msg.properties, {headers: {error: false, status: 'progress'}});
                enqueueFeedback.call(self, stringify(result), properties);
              }
            }
          });
        } else {
...
var getFeedbackState = function() {
  return (this.feedbackState = this.feedbackState || {});
}

var hasFeedback = function() {
  return this.config.feedback && (this.config.feedback.enabled !== false);
}

var assertFeedback = function() {
  if (!hasFeedback.call(this)) return Promise.reject({
    message: 'The feedback is unavailable'
  });
  return Promise.resolve({ config: this.config.feedback, store: getFeedbackState.call(this) });
}

var enqueueFeedback = function(data, opts) {
  ...
  return assertFeedback.call(self).then(function(vr) {
    ...
    return getChannel.call(vr).then(function(ch) {
      ...
      if (vr.config.prefetch && vr.config.prefetch >= 0) {
        debugx.enabled && debugx('pullout() - set channel prefetch: %s', vr.config.prefetch);
      ...
      var ok = assertQueue.call({ config: vr.config }, ch).then(function(qok) {
        ...
        if (vr.config.maxSubscribers && vr.config.maxSubscribers <= qok.consumerCount) {
          ...
          debugx.enabled && debugx('pullout() - Error: %s', JSON.stringify(error));
          return Promise.reject(error);
        }
        return Promise.promisify(ch.consume, {context: ch})(qok.queue, function(msg) {
          ...
          if (callback && typeof(callback) === 'function') {
              ...
              if (hasRecycler.call(self)) {
                ...
                if (err) {
                  ...
                  if (headers[rcfg.redeliveredCountName] <= rcfg.redeliveredLimit) {
                    ...
                    enqueueFeedback.call(self, msg.content.toString(), {headers: headers});
                  } else {
            ...
            debugx.enabled && debugx('pullout() - callback (observer) is not available');
          }
...
var recoverGarbage = garbageAction['recover'] = function() {
  ...
  return assertRecycler.call(self).then(function(vr) {
    ...
    return getChannel.call(vr).then(function(ch) {
      ...
      return enqueueConsumer.call(self, msg.content.toString(), msg.properties).then(function(result) {
...
var requeueGarbage = garbageAction['requeue'] = function() {
  ...
  return assertRecycler.call(self).then(function(vr) {
    ...
    return getChannel.call(vr).then(function(ch) {
      var msg = vr.store.garbage;
      return enqueueRecycler.call(self, msg.content.toString(), msg.properties).then(function(result) {
