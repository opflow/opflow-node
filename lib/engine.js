        ...
        if (callback && typeof(callback) === 'function') {
          callback(msg.content, {
            ...
            if (hasFeedback.call(self)) {
              ...
              enqueueFeedback.call(self, stringify(result || err || {}), {headers: headers});
...
var hasFeedback = function() {
  var feedbackCfg = this.config.feedback;
  return feedbackCfg && (feedbackCfg.enabled !== false);
}

var assertFeedback = function() {
  var feedbackCfg = this.config.feedback;
  if (!feedbackCfg || (feedbackCfg.enabled === false)) return Promise.reject({
    message: 'The feedback is unavailable'
  });
  return Promise.resolve({ config: feedbackCfg, store: getFeedbackState.call(this) });
}
