...
var Engine = function(params) {
  ...
  this.produce = function(body, props, override) {
    ...
    LX.isEnabledFor('verbose') && LX.log('verbose', {
      requestId: requestId,
      instanceId: misc.instanceId,
      note: 'produce() a message to exchange/queue'});
    var ok = lockProducer().then(function(sandbox) {
      ...
      if (sandbox.sendable !== false) {
        ...
        LX.isEnabledFor('verbose') && LX.log('verbose', {
          requestId: requestId,
          note: 'produce() channel is writable, msg has been sent'});
        return sandbox.sendable;
      } else {
        LX.isEnabledFor('info') && LX.log('info', {
          requestId: requestId,
          note: 'produce() channel is overflowed, waiting'});
        return new Promise(function(resolved, rejected) {
          sandbox.channel.once('drain', function() {
            sendTo();
            LX.isEnabledFor('info') && LX.log('info', {
              requestId: requestId,
              note: 'produce() channel is drained, flushed'});
  ...
  this.consume = function(callback, options) {
    ...
    return getConsumerState(options).then(function(sandbox) {
      ...
      LX.isEnabledFor('verbose') && LX.log('verbose', {
        callId: callId,
        note: 'consume() - create a consumer'});
      return getChannel(sandbox).then(function(ch) {
        ...
        ok = ok.then(function(qok) {
          if (options.binding === false) {
            LX.isEnabledFor('verbose') && LX.log('verbose', {
              callId: callId,
              queueName: qok.queue,
              note: 'consume() - queue is keep standalone'});
            return qok;
          }
          ...
          return Promise.resolve().then(function() {
            ...
            LX.isEnabledFor('verbose') && LX.log('verbose', {
              callId: callId,
              queueName: qok.queue,
              note: 'consume() - queue has been bound'});
            return qok;
          });
        });
        ...
        ok = ok.then(function(qok) {
          LX.isEnabledFor('verbose') && LX.log('verbose', {
            callId: callId,
            queueInfo: qok,
            note: 'consume() - queue ready to consume'});
          ...
          return ch_consume(qok.queue, function(msg) {
            ...
            LX.isEnabledFor('info') && LX.log('info', {
              ...
              note: 'consume() receive new message'});
        ...
        return ok.then(function(result) {
          LX.isEnabledFor('verbose') && LX.log('verbose', {
            ...
            note: 'consume() is running. CTRL+C to exit'});
