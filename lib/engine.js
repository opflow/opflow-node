...
var CouplingStream = require('./task').CouplingStream;
...
var Engine = function(params) {
  ...
  if (params.confirmation && typeof(params.confirmation) === 'object') {
    ...
    if (!lodash.isNumber(config.confirmation.redeliveredLimit)) {
  ...
  var producerTasks = {};
  var producerTimeoutHandler = null;

  var initTimeoutHandler = function() {
    var confirmConfig = config.confirmation || {};
    var timeoutHandler = new TimeoutHandler({
      monitorId: engineId,
      monitorType: 'confirmation',
      interval: confirmConfig.interval || 7000,
  ...
  this.produce = function(body, props, override) {
    ...
    var ok = obtainProducerSandbox().then(function(sandbox) {
      ...
      if (sandbox.confirmable) {
        producerTimeoutHandler = producerTimeoutHandler || initTimeoutHandler();
        producerTasks[requestId] = { body: body, props: props, override: override,
          timestamp: misc.getCurrentTime()
        }
      }
      var sendTo = function() {
        sandbox.sendable = sandbox.channel.publish(config.exchangeName, 
          override.routingKey || config.routingKey, misc.bufferify(body), props, function(err, ok) {
            if (err) {
              LX.isEnabledFor('debug') && LX.log('debug', requestTrail.add({
                message: 'produce() confirmation has failed'
              }).toString());
            } else {
              delete producerTasks[requestId];
              LX.isEnabledFor('debug') && LX.log('debug', requestTrail.add({
  ...
  this.exhaust = function(stream, props, override) {
    if (!(stream instanceof Readable)) return Promise.reject({
      message: '[stream] is not a readable stream'
    });
    override = override || {};
    props = props || {};
    props.appId = props.appId || config.applicationId;
    ...
    var ok = obtainProducerSandbox().then(function(sandbox) {
      LX.isEnabledFor('info') && LX.log('info', requestTrail.add({
        message: 'exhaust() a stream to exchange/queue',
        producerId: sandbox.id
      }).toString());
      ...
      var sendTo = function(body) {
        var objectId = misc.getLogID();
        var objectTrail = requestTrail.branch({ key: 'objectId', value: objectId });
        if (sandbox.confirmable) {
          producerTasks[objectId] = { body: body, props: props, override: override,
            timestamp: misc.getCurrentTime()
          }
          LX.isEnabledFor('debug') && LX.log('debug', objectTrail.add({
            message: 'exhaust() add new object to confirmation list'
          }).toString());
        }
        return sandbox.channel.publish(config.exchangeName,
          override.routingKey || config.routingKey, misc.bufferify(body), props, function(err, ok) {
            if (err) {
              LX.isEnabledFor('debug') && LX.log('debug', objectTrail.add({
      ...
      var producerStream = new CouplingStream(function onNext(body, index, notifier) {
        var drained = sendTo(body);
        if (!drained) {
          sandbox.channel.once('drain', function() {
            notifier.emit('drain');
          });
      ...
      return new Promise(function(onResolved, onRejected) {
        LX.isEnabledFor('debug') && LX.log('debug', requestTrail.add({
          message: 'exhaust() streaming begin'
        }).toString());
        producerStream.on('finish', function() {
          LX.isEnabledFor('debug') && LX.log('debug', requestTrail.add({
            total: producerStream.count(),
            message: 'exhaust() streaming end'
          }).toString());
          onResolved();
        });
        producerStream.on('error', function(err) {
          LX.isEnabledFor('error') && LX.log('error', requestTrail.add({
            ...
            message: 'exhaust() streaming error'
          }).toString());
          onRejected(err);
        });
  ...
  this.close = function() {
    ...
    return Promise.resolve().then(function() {
      LX.isEnabledFor('info') && LX.log('info', closeTrail.add({
        message: 'close() - stop confirmation'
      }).toString());
      if (producerTimeoutHandler === null) return true;
      var result = producerTimeoutHandler.stop();
      producerTimeoutHandler = null;
      return result;
    }).then(function() {
      LX.isEnabledFor('info') && LX.log('info', closeTrail.add({
  ...
  var obtainProducerSandbox = function() {
    ...
    if (!producerSandbox.channel && producerSandbox.mutex) {
  ...
  var obtainConsumerSandbox = function(opts) {
    ...
    return lockConsumer(consumerSandbox).then(function() {
      if (!opts) {
        ...
        return Promise.resolve(consumerSandbox);
      }
