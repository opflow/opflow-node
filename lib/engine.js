  ...
  this.produce = function(bodyOrStream, props, override) {
    ...
    var ok = obtainProducerSandbox().then(function(sandbox) {
      ...
      var sendTo = function(body, index) {
        var segmentId = (index === null) ? requestId : misc.getLogID();
        var segmentTrail = (index === null) ? requestTrail : requestTrail.branch({
      ...
      var doSend = function(body, index) {
        if (sandbox.sendable !== false) {
          sandbox.sendable = sendTo(body, index);
      ...
      if (bodyOrStream instanceof Readable) {
        var producerStream = new CouplingStream(doSend, { streamId: requestId });
        ...
        return new Promise(function(onResolved, onRejected) {
          ...
          bodyOrStream.pipe(producerStream, { end: true });
        });
      } else {
