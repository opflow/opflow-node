        ...
        if (callback && typeof(callback) === 'function') {
          ...
          callback(msg.content, {
            ...
            if (hasFeedback.call(self)) {
              ...
              var properties;
              if (err) {
                properties = lodash.defaultsDeep({headers: {error: true, status: 'failed'}}, msg.properties);
                debugx.enabled && debugx('consume() - processing failed: %s', stringify(err));
              } else {
                properties = lodash.defaultsDeep({headers: {error: false, status: 'completed'}}, msg.properties);
              ...
              if (hasRecycler.call(self)) {
                ...
                if (err) {
                  ...
                  if (props.headers[rcfg.redeliveredCountName] <= rcfg.redeliveredLimit) {
                    ...
                    enqueueConsumer.call(self, msg.content, props);
                  } else {
                    debugx.enabled && debugx('consume() - enqueueRecycler message');
                    enqueueRecycler.call(self, msg.content, props);
                  }
                }
                ch.ack(msg);
              } else {
