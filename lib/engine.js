...
var Engine = function(params) {
  ...
  var closeChannel = function(sandbox) {
    if (sandbox.channel) {
      ...
      return ch_close().then(function() {
        ...
        LX.isEnabledFor('verbose') && LX.log('verbose', 'closeChannel() - channel is closed');
        return true;
      });
    }
    return Promise.resolve(true);
  ...
  var getConsumerSandboxName = function(sharingLevel) {
    return self.consumerState.name + '[' + sharingLevel + ']' + (new Date()).toISOString();
  }
