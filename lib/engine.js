...
var Engine = function(params) {
  ...
  this.cancelConsumer = function(sandbox) {
    return Promise.resolve().then(function() {
      ...
      LX.isEnabledFor('verbose') && LX.log('verbose', 'cancelConsumer() - skip close channel');
      return true;
    }).then(function() {
      if (sandbox.forceNewConnection) return closeConnection(sandbox);
      LX.isEnabledFor('verbose') && LX.log('verbose', 'cancelConsumer() - skip close connection');
      return true;
    });
  }
  ...
  this.close = function() {
    return Promise.resolve().then(function() {
      ...
      LX.isEnabledFor('verbose') && LX.log('verbose', 'close() - close channel');
      return closeChannel(getConsumerState());
    }).then(function() {
      LX.isEnabledFor('verbose') && LX.log('verbose', 'close() - close connection');
  ...
  var getConnection = function(sandbox) {
    ...
    LX.isEnabledFor('verbose') && LX.log('verbose', 'getConnection() - connection amount: %s', sandbox.connectionCount);
  ...
  var closeConnection = function(sandbox) {
    ...
    if (sandbox.connection) {
      ...
      return ch_close().then(function(ok) {
        ...
        LX.isEnabledFor('verbose') && LX.log('verbose', 'closeConnection() - connection is closed: %s', JSON.stringify(ok));
        return true;
      });
    }
  ...
  var getChannel = function(sandbox, opts) {
    ...
    if (config.channelIsCached !== false && sandbox.channel) {
      LX.isEnabledFor('verbose') && LX.log('verbose', 'getChannel() - channel has been available (%s)', opts.name);
      return Promise.resolve(sandbox.channel);
    } else {
      LX.isEnabledFor('verbose') && LX.log('verbose', 'getChannel() - make a new channel (%s)', opts.name);
      return getConnection(sandbox).then(function(conn) {
        LX.isEnabledFor('verbose') && LX.log('verbose', 'getChannel() - connection has already (%s)', opts.name);
        ...
        return createChannel().then(function(ch) {
          ...
          LX.isEnabledFor('verbose') && LX.log('verbose', 'getChannel() - channel is created (%s)', opts.name);
  ...
  var closeChannel = function(sandbox) {
    if (sandbox.channel) {
      ...
      return ch_close().then(function(ok) {
        ...
        LX.isEnabledFor('verbose') && LX.log('verbose', 'closeChannel() - channel is closed: %s', JSON.stringify(ok));
        return true;
      });
    }
    return Promise.resolve(true);
