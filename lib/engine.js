...
var Engine = function(params) {
  ...
  this.cancelConsumer = function(sandbox) {
    ...
    return Promise.resolve().then(function() {
      ...
      return getConsumerState().then(function(sharedSandbox) {
        return Promise.resolve().then(function() {
  ...
  var getProducerState = function() {
    ...
    if (!self.producerState.id) {
      self.producerState.id = misc.getLogID();
  ...
  var getConsumerState = function(opts) {
    ...
    if (!self.consumerState.id) {
      self.consumerState.id = misc.getLogID();
      LX.isEnabledFor('conlog') && LX.log('conlog', engineTrail.reset().add({
    ...
    self.consumerState.mutex = self.consumerState.mutex || locks.createMutex();

    var consumeTrail = engineTrail.copy();
    return lockConsumer(self.consumerState).then(function() {
      if (!opts) {
        LX.isEnabledFor('info') && LX.log('info', consumeTrail.add({
      ...
      unlockConsumer(self.consumerState);
    });
  }
  ...
  var unlockConsumer = function(consumerSandbox) {
    consumerSandbox.mutex && consumerSandbox.mutex.unlock();
  }
